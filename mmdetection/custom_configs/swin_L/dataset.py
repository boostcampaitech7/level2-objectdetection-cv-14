...
albu_train_transforms = [
    ...
    dict(type='OneOf',
        transforms=[
            ...
            dict(type='RandomFog',fog_coef_lower=0.3, fog_coef_upper=0.5,   p=1.0),
            dict(type='RGBShift',r_shift_limit=10,g_shift_limit=10,b_shift_limit=10,p=1.0)],p=0.5),
    dict(type='OneOf',
        transforms=[
            dict(type='Blur', blur_limit=3, p=1.0),
            dict(type='MedianBlur', blur_limit=3, p=1.0)],p=0.5)
]

train_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(type='LoadAnnotations', with_bbox=True),
    dict(type='Resize', img_scale=[(2000,480), (2000,1200)], keep_ratio=True),
    dict(
                type='Albu',
